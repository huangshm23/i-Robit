...
export default {
  ...
  methods:{
    logout:function(){
      ...
      this.$router.push('/')
      //发送退出请求到后端，返回0成功, 1失败
      this.$http.get(this.$store2.state.basicUrl + 'logout/?token=' + this.$store.state.token).then(function(res){
    ...
    next:function(){
      this.$http.get(this.$store2.state.basicUrl + 'news/?token=' + this.$store.state.token).then(function(res){
