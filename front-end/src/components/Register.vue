...
export default {
  ...
  this.$http.post(this.$store.state.basicUrl + 'register/',{username:this.form.username,password:this.form.password},{emulateJSON:true}).then(function(res){
  ...
  deactivate:function(){
    this.$http.get(this.$store.state.basicUrl + 'activate/<activate_id>').then(function(res){
                    if (res.body.status)
                      console.log('激活失败');
                    else  
                      activate = 0;
